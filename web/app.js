...
const FACTORS = [
  { key: 'park', label: 'Park', kind: 'near', unit: 'm', r: 800, rmax: 3000, w: 1.0, on: true, desc: 'Stroller walks. OSM parks, adjacent pieces merged; the size slider sets what counts.' },
  { key: 'playground', label: 'Playground', kind: 'near', unit: 'm', r: 400, rmax: 2000, w: 0.5, on: false, desc: 'Relevant from ~1 year on.' },
  { key: 'supermarket', label: 'Supermarket', kind: 'near', unit: 'm', r: 500, rmax: 2000, w: 0.8, on: true, desc: 'Everyday shopping on foot.' },
  { key: 'mall', label: 'Shopping mall', kind: 'near', unit: 'm', r: 2500, rmax: 8000, w: 0.6, on: true, desc: 'Parque Principado, Los Prados, Salesas, Azabache… (OSM shop=mall).' },
  { key: 'health_centre', label: 'Centro de salud', kind: 'near', unit: 'm', r: 1000, rmax: 4000, w: 0.7, on: true, desc: 'Public primary care: the paediatrician for checkups and vaccinations.' },
  { key: 'hospital', label: 'Hospital', kind: 'near', unit: 'm', r: 3000, rmax: 10000, w: 0.5, on: true, desc: 'HUCA and Centro Médico de Asturias (paediatric emergencies).' },
  { key: 'bus', label: 'Bus stop', kind: 'near', unit: 'm', r: 400, rmax: 2000, w: 0.8, on: true, desc: 'Any bus stop. Dense inside Oviedo, sparse outside.' },
  { key: 'rail', label: 'Train station', kind: 'near', unit: 'm', r: 1500, rmax: 5000, w: 0.6, on: true, desc: 'Renfe / Cercanías stations and halts (Oviedo, Lugones, Colloto, Meres…).' },
  { key: 'kindergarten', label: 'Daycare (escuela infantil)', kind: 'near', unit: 'm', r: 800, rmax: 3000, w: 0.5, on: false, desc: 'OSM amenity=kindergarten; coverage may be partial.' },
  { key: 'rent', label: 'Rent (€/m²)', kind: 'range', unit: '€/m²', lo: 9.5, r: 12.5, rmin: 10, rmax: 15, w: 0.6, on: true, desc: 'idealista district averages, Aug 2026. Score 1 at 9.5 €/m², 0 at the slider value. Near-flat across Oviedo (10.3–12.4).' },
  { key: 'slope', label: 'Hilliness (slope %)', kind: 'low', unit: '%', r: 12, rmin: 3, rmax: 30, w: 1.0, on: true, desc: 'Mean terrain gradient within ~200 m (SRTM 30 m). Score 0 at the slider value.' },
  { key: 'industrial', label: 'Industrial zone', kind: 'far', unit: 'm', r: 500, rmax: 2000, w: 0.7, on: true, desc: 'Distance to the nearest industrial area (OSM landuse).' },
  { key: 'big_road', label: 'Motorway / main road', kind: 'far', unit: 'm', r: 200, rmax: 1000, w: 0.6, on: true, desc: 'Traffic-noise proxy until the official noise map is loaded.' },
  { key: 'railway', label: 'Railway line', kind: 'far', unit: 'm', r: 150, rmax: 800, w: 0.4, on: true, desc: 'Distance to the nearest rail track.' },
  { key: 'noise_lden', label: 'Noise, day-evening-night (Lden)', kind: 'range', unit: 'dB', lo: 50, r: 65, rmin: 55, rmax: 80, w: 0.8, on: true, desc: 'Official strategic noise map (MITECO, phase 3), band lower bounds averaged over the cell. Score 1 below 55 dB, 0 at the slider value. Oviedo municipality only; elsewhere only the road/rail proxies apply.' },
  { key: 'noise_ln', label: 'Noise, night (Ln)', kind: 'range', unit: 'dB', lo: 45, r: 55, rmin: 50, rmax: 75, w: 0.6, on: false, desc: 'Same map, night indicator. Score 1 below 50 dB.' },
  { key: 'reputation', label: 'Barrio reputation', kind: 'score', unit: '', w: 1.0, on: true, desc: 'From the researched dossiers (data/manual/dossiers.json): safety, upkeep, social issues, what locals say. Click a cell, then "dossier" to read it. Override a score in the JSON and rebuild.' },
  { key: 'flood', label: 'Flood zone', kind: 'flood', unit: '', w: 0.7, on: true, desc: 'Official flood zones (MITECO SNCZI), averaged over the cell: 100-year zone counts 2, 500-year zone 1. Score = 1 − level/2. Matters for low floors.' },
];
